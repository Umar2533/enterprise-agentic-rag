...
def _activation_toast(message: str, icon: str) -> None:
    # toast_icon = "✅" if icon == "?" else "⚠️" if icon in {"??", ""} else icon
    icon_map = {
    "success": "✅",
    "warning": "⚠️",
    "error": "❌",
    ...
    toast_icon = icon_map.get(icon, icon)
    if hasattr(st, "toast"):
...
def _attach_chat_collection(selected: dict) -> None:
    ...
    try:
        ...
        _activation_toast("Could not activate collection. Please try again.", "warning")
    ...
    if error:
        ...
        with button_col:
            ...
            if st.button("Activate", key="chat_activate_collection", width="stretch"):
                _attach_chat_collection(selected)
...
def _render_chat_bottom_spacer() -> None:
    st.markdown('<div class="chat-bottom-safe-space" aria-hidden="true"></div>', unsafe_allow_html=True)
...
def render_header_toolbar() -> None:
    st.markdown(
        ...
        .chat-header-shell {
            ...
            padding: 0 !important;
            margin: -8px 0 4px 0 !important; /* top gap remove */
        }
        ...
        .chat-header-inner {
            ...
            margin: 0 !important;
            padding: 0 88px 0 0 !important; /* right sidebar space */
...
def render_chat_layout():
    """Return the center chat column while rendering the right tools drawer."""
    collapsed = bool(st.session_state.get("chat_right_drawer_collapsed", False))

    width_var = "72px" if collapsed else "320px"
    st.markdown(f'<style>.stApp:has(.chat-page-root) {{ --right-panel-width: {width_var}; }}</style>', unsafe_allow_html=True)

    right_weight = 0.08 if collapsed else 0.30
...
def _render_right_workspace_panel() -> None:
    ...
    with st.container(key="chat_right_panel"):
        st.markdown(f'<span class="chat-right-panel-state {state_class}" aria-hidden="true"></span>', unsafe_allow_html=True)
        toggle_label = "<" if not collapsed else ">"
        if st.button(toggle_label, key="chat_right_drawer_toggle", help="Toggle right drawer"):
            toggle_right_drawer()
            st.rerun()

        if collapsed:
            st.markdown(
                '<div class="chat-right-icon-rail">'
                + "".join(
                    f'<span class="chat-right-icon" title="{html.escape(title)}">{icon}</span>'
        ...
        st.markdown(
            """
            <div class="chat-right-panel-head">
              <strong>Studio</strong>
              <span>Tools and sources</span>
            </div>
            """,
            unsafe_allow_html=True,
        )
        _render_export_popover()
        st.markdown('<div class="chat-right-sources-action">', unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            + "".join(
                f'<div class="chat-tool-card tool-{index}"><span>{icon}</span><strong>{html.escape(title)}</strong><em>Coming soon</em></div>'
                for index, (title, icon) in enumerate(tools)
            )
            + "</div>",
            unsafe_allow_html=True,
        )
...
def _render_chat_composer(active_collection: str | None) -> tuple[str, bool]:
    ...
    with st.form(key="chat_composer_form", clear_on_submit=True):
        web_col, input_col, send_col = st.columns([0.08, 1, 0.08], gap="small", vertical_alignment="center")
        with web_col:
...
if question:
    ...
    with messages_slot:
        ...
        progress = st.empty()
        try:
            progress.caption("Searching selected collection...")
            for event, payload in chat_stream(
                ...
                if event == "sources":
                    ...
                    placeholder.markdown(_bubble_html("".join(answer_parts), "assistant", streaming=True), unsafe_allow_html=True)
                elif event == "done":
